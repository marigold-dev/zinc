...
let of_yojson = function
  ...
  let pp : Format.formatter -> t -> unit = fun fmt v -> Format.fprintf fmt "%s" (Z.to_string v)
end
...
module LMap = struct 
  ...
  let pp : (Format.formatter -> 'a -> unit) -> Format.formatter -> 'a t -> unit = fun value ppf m -> 
    let lst = bindings m in
    let lst = Base.List.dedup_and_sort ~compare:(fun (Label a,_) (Label b,_) -> String.compare a b) lst in
    let new_pp ppf (k, v) = Format.fprintf ppf "@[<h>%a -> %a@]" (pp_label) k value v in
    Format.fprintf ppf "%a" (Format.pp_print_list ?pp_sep:(Some (fun fmt _ -> Format.pp_print_string fmt ", ")) new_pp) lst
